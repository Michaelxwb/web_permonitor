"""FastAPI profile decorator for function-level monitoring.
...
class FastAPIProfileDecorator(BaseDecorator):
    """FastAPI-specific function profiling decorator.
    ...
    If called within a FastAPI request context, additional context
    ...
    def _get_context(self) -> Dict[str, Any]:
        """Get FastAPI-specific context information.

        Attempts to get request context if available via contextvars.

        Returns:
            Dictionary with context data from FastAPI request, if available.
        """
        context: Dict[str, Any] = {}

        try:
            # Try to get request from Starlette's context
            from starlette.requests import Request

            from .middleware import get_current_profiler

            # If we're in a middleware context, we might have request info
            # but decorators are typically used outside middleware context
            # So we just return an empty context for now

        except ImportError:
            logger.debug("Starlette not available for context extraction")
        except Exception as e:
            logger.debug(f"Error extracting FastAPI context: {e}")
